...
| 생활사건 선택 | `LifeChoice = 'cash' \| 'deposit' \| 'withdraw' \| 'contribute-all' \| 'contribute-half' \| 'transfer-irp'`(`cash`는 모든 사건에서 "IRP는 그대로, 생활자금 쪽으로"). 비용 사건: `cash`(생활자금, 모자라면 기존 자동 충당) · `deposit`(예금 중도해지로 지급, 보유 10만 이상일 때만, 불이익 1.2%, 모자라면 생활자금) · `withdraw`(허용 사유만, 수수료 3% 유지; 불가 사유는 **비활성 + 이유 표시**). 보너스: `contribute-all` · `contribute-half`(연간 한도 안) · `cash`. 이직 퇴직금: `transfer-irp`(IRP 대기자금으로 전액, 세금 없음) · `cash`(지금 받기 — 일시 수령, 교육용 16.5% 차감) |
...
| 기준선(고스트·`passive`) | 모든 사건을 `cash`로 푼다 — 지출은 생활자금, 보너스는 생활자금에 두고, **퇴직급여도 지금 받는다**(현실에서 퇴직급여 IRP 계좌 대다수가 곧 해지되는 것을 따름). 그래서 IRP 이전은 "판단"으로 남고 `passive` 0별 비율(≥ 0.3)이 유지된다. 측정: 퇴직급여를 기준선이 IRP로 옮기면 `passive` 0별 0.383 → 0.297로 무너져 게이트 실패, 지금 받기로 두면 0.383 그대로 |
