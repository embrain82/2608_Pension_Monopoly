...
- **점프 재설계(2차).** 위 수정 뒤에도 "칸마다 점프"로 읽히지 않는다는 보고. 포물선 높이(퍽의 58%≈33px)가 가로 이동(105px)에 비해 낮아 평평한 활주로 보였고, 칸 사이에 멈춤이 없어 한 번의 글라이드로 이어졌다. `hopPlan(final)` 하나로 한 칸 점프를 정의: 260ms 중 70%(182ms) 공중(최고점 = 퍽 높이 125%, 가로 이동은 공중에서만 `slideKeyframes`), 칸에 닿으며 작은 찌그러짐, 30%는 칸 위에 머무는 박자. 마지막 칸은 같은 점프에 큰 찌그러짐(320ms)이 이어져 한 애니메이션으로 끝난다(`onfinish` 연쇄 제거). 검증은 헤드리스 스크린캐스트가 아니라 **실제 X 디스플레이 30fps 캡처**(`ffmpeg x11grab` + headful Chrome)로 했다: 헤드리스 스크린캐스트는 컴포지터 애니메이션 중간 프레임을 거의 잡지 못해 점프가 없는 것처럼 보였다(영상 리뷰 모델도 같은 이유로 "평평한 슬라이드"로 오판). 실제 화면 캡처의 여우 색 중심 추적: 3칸·10칸 이동 모두 칸마다 최고점 72~73px 상승, 칸 위 멈춤 2~3프레임, 마지막 칸 포함 동일.
