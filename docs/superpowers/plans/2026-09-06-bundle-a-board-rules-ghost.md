...
- [x] 1. 플랜 문서(이 파일)
- [x] 2. 3.2 턴 순서 — 엔진(`startTurn`·`finalizeTurn`·장부)·`summarizeTurn` 3점·`newsChaser`·테스트·게이트 측정
- [x] 3. 3.2 UI — 정산 막대 3개·블록 재배치, 속보 IRP 줄·코치, 행동 메뉴 문구, 하우투, 학습 카드
- [x] 4. 3.1 엔진 — `tile-effects.ts`, `boardTiles.effect`, 환급 보류·통과 환급, 행동 2회, 스포트라이트, 추가 사건, 전망대, 성향 점검, 테스트·게이트
- [x] 5. 3.1 UI — 속보 도착 효과 스트립, 행동 시트 "행동 1/2", 리밸런싱 지금→목표, 정산 칸 효과 블록, 환급 연출·소리
- [x] 6. 3.3 — 고스트 계산·저장 v4·정산 줄·결과 스파크라인 2줄·값어치·HUD 마커·설정
- [x] 7. 매뉴얼(사용자·운영자) 현행화 + QA 케이스(Q37~Q45, C30~C37) + 다이어그램 5장 갱신(archify showcase·visual-check 통과)
- [x] 8. `npm run typecheck && npm test && npm run build`, 브라우저 확인(CDP 12턴 완주·스크린샷), 커밋·PR #12

## 구현 중 바뀐 것(플랜 대비)

- 장부는 `turnOpenIrp`·`turnMarketIrp` 두 필드 대신 `GameState.ledger { open, afterMarket, beforeAction }` 하나로 묶었다. `beforeAction`은 첫 행동 직전 스냅샷(IRP·위험비중·보유 맵)이라 행동 2회 턴에도 "내 행동" 델타가 한 번에 잡힌다.
- 생활사건이 있는 턴은 `TurnSummary.lifeDelta`를 따로 떼어 "시장 ±N · 내 행동 ±M · 생활사건 −K" 세 항의 합이 전체 변화와 같게 했다.
- `newsChaser`는 "ETF +1% 넘으면 예금→ETF 교체, −1% 밑이면 ETF→예금, 그 외 납입"으로 두었다(3.2 이전 최적해 재현).
- 결과 스파크라인 끝 라벨은 흰 외곽선(`paint-order: stroke`)과 8px 여백으로 점선과 겹치지 않게 했다.
- 매뉴얼 현행화는 이후 모든 PR의 필수 절차로 운영자 매뉴얼 §5와 체크리스트에 적었다.

## 밸런스 게이트 측정값 (시드 400 `play-0..399`, 2026-09-06)

기준: `main 9624cb0`(태그 `prod-2026-09-06-manual-diagrams`). 스크립트는 `/tmp/sim/baseline.ts`·`gate-a.ts`(저장소 밖).

| 전략 | 기준 월연금 | 켬 월연금 | 기준 별 | 켬 별 | 기준 3별 | 켬 3별 | 기준 0별 | 켬 0별 | 낙폭(켬) |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| balanced | 506,418 | 509,085 | 2.25 | 2.37 | 63% | 70% | 1% | 2% | 1.8% |
| passive | 479,415 | 479,415 | 0.68 | 0.68 | 0% | 0% | 41% | 41% | 2.1% |
| contributor | 529,415 | 533,778 | 1.91 | 1.63 | 0% | 0% | 0% | 0% | 0.9% |
| growth | 505,586 | 507,755 | 2.13 | 2.19 | 54% | 54% | 0% | 0% | 1.7% |
| steward | 516,721 | 514,923 | 2.90 | 2.87 | 94% | 94% | 0% | 0% | 1.2% |
| etfOnly | 492,156 | 492,626 | 0.98 | 0.99 | 0% | 0% | 43% | 41% | 8.6% |
| stopLoss | 520,424 | 499,880 | 1.58 | 0.96 | 0% | 0% | 8% | 31% | 6.9% |
| momentum | 490,263 | 490,425 | 1.07 | 1.06 | 0% | 0% | 19% | 19% | 2.0% |
| newsChaser | — | 484,308 | — | 0.88 | — | 0% | — | 44% | 6.9% |

칸 효과 끔(턴 순서만 바꾼 상태)은 `passive`·`contributor`가 기준과 동일(479,415 / 529,415), `balanced` 507,521, `newsChaser` 481,841.

- 3.2 `newsChaser > balanced` **21%** (≤ 50% ✓). `passive` 0.0%, `contributor` +0.8% (±1% ✓). `momentum` 1.07 → 1.06, `stopLoss` 1.58 → 0.96 (오르지 않음 ✓). `stopLoss`가 크게 내린 것은 속보를 보고 파는 손절이 이미 반영된 뒤라 "하락 뒤 매도"가 재진입 없는 현금화가 되기 때문 — 의도한 방향.
- 3.1 `passive` 0별 41% (≥ 35% ✓). `steward` 3별 94% → 94%, `balanced` 63% → 70% (±10%p ✓). 칸 효과 켬/끔 월 연금 차이 최대 +4,363원(`contributor`) (≤ 1.5만 ✓). 같은 시드 → 같은 효과는 `tests/tile-effects.test.ts` 결정성 케이스로 고정.
- `contributor` 별 1.91 → 1.63: 환급이 판 중간에 생활자금으로 돌아오면 전략이 그 돈을 다시 납입해 마감 생활자금이 600만 원(2별 조건) 아래로 내려가는 시드가 늘었다(`/tmp/sim/contrib.ts`). 총자산·월 연금은 늘었으므로 밸런스 결함이 아니라 "전부 납입" 전략의 약점이 드러난 것. 그대로 둔다.
- 3.3 고스트 최종값 = `autoplay(seed, 'passive')` 최종값 — `tests/ghost.test.ts`.
- `tests/balance-gate.test.ts` 전 케이스 통과(전략 9종 × 100판).
