...
export const ACHIEVEMENTS: AchievementDef[] = [
  ...
  { id: 'diversified-12', title: '분산 12턴', detail: `매 턴 마감에 5% 이상 보유 상품이 ${balanceConfig.diversificationMin}종 이상이었다.`, scope: 'game' },
...
const GAME_CHECKS: Record<Exclude<AchievementId, 'all-profiles'>, (state: GameState, score: ScoreResult) => boolean> = {
  ...
  'diversified-12': (state) => state.record.diversifiedTurns >= balanceConfig.maxTurns,
  ...
  'default-option-run': (state) => state.record.defaultOptionRuns >= DEFAULT_OPTION_RUNS,
