...
export function renderIrpSparkline(history: number[], shockTurns: number[], ghostHistory: number[] | null = null): string {
  ...
  if (ghost) {
    ...
    labels = `<text class="chart-label me" x="${end.x - 4}" y="${myY}" text-anchor="end">내 판단</text><text class="chart-label ghost" x="${ghostEnd.x - 4}" y="${ghostY}" text-anchor="end">그대로 둔 나</text>`;
  }
